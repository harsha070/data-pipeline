...
def load_data(local_file_path: str) -> pd.DataFrame:
    ...
    """
    assert local_file_path is not None, f"file path cannot be None!"
    assert os.path.exists(local_file_path), f"file path {local_file_path} does not exist!"
    if local_file_path.endswith(".xlsx"):
        return pd.read_excel(local_file_path)
    elif local_file_path.endswith(".csv"):
        return pd.read_csv(local_file_path)
    elif local_file_path.endswith(".tsv"):
...
def get_added_deleted_data(data_1, data_2, key_columns):
    ...
    added_df = data_2.merge(data_1, on=key_columns, how='outer', indicator=True).query("_merge == 'right_only'")
    removed_df = data_1.merge(data_2, on=key_columns, how='outer', indicator=True).query("_merge == 'left_only'")
    del added_df["_merge"]
